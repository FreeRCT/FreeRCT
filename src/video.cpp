 ...
 * @param padding Include text padding in the result.
 * @param scale Scaling factor for the text size.
 */
PointF TextRenderer::EstimateBounds(const std::string &text, bool padding, float scale) const
{
	float x = 0;
	float width = 0;
	float height = padding ? this->font_size * scale : 0;
	size_t text_length = text.size();
	for (const char *c = text.c_str(); *c != '\0';) {
		const FontGlyph &fg = this->GetFontGlyph(&c, text_length);
...
void VideoSystem::MouseClickCallback([[maybe_unused]] GLFWwindow *window, int button, int action, [[maybe_unused]] int mods) {
	...
	MouseButtons mouse;
	switch (button) {
		case GLFW_MOUSE_BUTTON_RIGHT : mouse = MB_RIGHT ; break;
		case GLFW_MOUSE_BUTTON_LEFT  : mouse = MB_LEFT  ; break;
		case GLFW_MOUSE_BUTTON_MIDDLE: mouse = MB_MIDDLE; break;
		default:
			return;
	}

	_window_manager.MouseButtonEvent(mouse, action == GLFW_PRESS ? WMEM_PRESS : WMEM_RELEASE);
}
 ...
 * @param padding Include text padding in the result.
 */
void VideoSystem::GetTextSize(const std::string &text, int *width, int *height, bool padding)
{
	if (width == nullptr && height == nullptr) return;
	PointF vec = _text_renderer.EstimateBounds(text, padding);
