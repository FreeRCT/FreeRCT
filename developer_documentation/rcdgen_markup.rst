...
These files are generated by the C preprocessor out of
**FreeRCT Markup Language (FML)** files and **FreeRCT Markup Snippet (FMS)** files.

The three file formats differ only in that FPP files may not contain `preprocessor directives`_ except `line directives`_.
...
Each FMS file is included only one file.
...
The ``hor`` sequence generates the columns of the matrix, and the ``hor`` sequence the rows of the matrix.
...
~~~~~~~~~~~~~~~~~~~~~~~
FML and FMS files may contain any directive understood by the C Preprocessor. FPP files may not contain such directives.
